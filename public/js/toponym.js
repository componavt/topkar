...
function saveSettlement() {
    ...
    var data = {
        'name_ru': $("#modalAddSettlement #name_ru").val(),
...
function addStruct(structhier_id) {
    ...
    if (structhier_id) {
        ...
        if (current_structhier) {
            $('#modalAddStruct #structhier_id').val(current_structhier);
        }
    }

    $('#modalAddStruct').modal('show');
}

function saveStruct() {
    var name_ru = $('#modalAddStruct #name_ru').val();
    var structhier_id = $('#modalAddStruct #structhier_id').val();
    var route = '/misc/structs/simpleStore';
    var locale = $("#locale").val();

    // Делаем кнопку неактивной
    $("#modalAddStruct .modal-footer button[type='submit']").prop('disabled', true);

    $.ajax({
        url: route,
        data: {
            name_ru: name_ru,
            structhier_id: structhier_id,
            locale: locale
        },
        type: 'GET',
        success: function (struct) {
            if (struct) {
                // Определяем, какой элемент формы мы обрабатываем (топоним или улицу)
                // Определяем тип формы по наличию селектора .select-struct0 (который есть только в топонимах)
                var isToponym = ($(".select-struct0").length > 0);

                if (isToponym) {
                    // Для топонимов
                    // Обновляем селектор структуры
                    var opt = new Option(struct['name'], struct['id']);
                    $(".select-struct0").append(opt);
                    opt.setAttribute('selected', 'selected');

                    // Обновляем селектор иерархии структуры
                    $("#structhiers_0_").val(struct['structhier_id']);
                } else {
                    // Для улиц - обновляем соответствующий селектор структуры
                    var structSelector = ".select-struct" + structhier_id;
                    var opt = new Option(struct['name'], struct['id']);
                    $(structSelector).append(opt);
                    opt.setAttribute('selected', 'selected');
                }
            }
            $("#modalAddStruct").modal('hide');
        },
        error: function (jqXHR, textStatus, errorThrown) {
            var text = 'Ajax Request Error: ' + 'XMLHTTPRequestObject status: (' + jqXHR.status + ', ' + jqXHR.statusText + '), ' +
                'text status: (' + textStatus + '), error thrown: (' + errorThrown + '), route: ' + route;
            alert(text);
            // Активируем кнопку снова в случае ошибки
            $("#modalAddStruct .modal-footer button[type='submit']").prop('disabled', false);
        }
    });
}
